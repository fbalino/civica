...
- **Reader-facing surfaces are Fable-led by default.** Workers implement UI only against an explicit design brief that names the DESIGN.md layout row, hero treatment, and component register. Absent that brief, stop at the data/API seam and leave the surface for the design pass; a working-but-undesigned flagship surface is a defect, not a deliverable.
