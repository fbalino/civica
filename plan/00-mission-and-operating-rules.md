...
- **The token ratchet catches raw values only; composition drift is still drift.** A canonical primitive restyled by an enclosing cascade rule, a layout register too modest for the surface's role, a grid change that alters content anchoring, or a hardcoded default entity are all design-system violations even when every value is a token. Browser QA that verifies function without comparing the surface to `/design-system` register does not satisfy the UI checks.
- **Reader-facing surfaces ship only against an explicit design brief** that names the DESIGN.md layout row, hero treatment, and component register — this binds whichever agent implements the surface. Absent that brief, stop at the data/API seam and queue the surface for a dedicated design pass; a working-but-undesigned flagship surface is a defect, not a deliverable.
- **Check these recorded failure modes before shipping any reader surface** (2026-07-11 owner drift review): `.editorial-section a` recoloring `.btn` text via cascade specificity; the masthead caption grid row stretching and vertically centering hero content; a flagship navigation destination shipped on the plain tool-page shell with a hardcoded default country; ranking columns surfacing whichever source data happened to load, unlabeled, instead of a deliberate named source.
