...
**Decision:** `civica-atlas-export/v2` publishes one gzip-compressed JSON package for release `atlas-2026-07-11`. It contains stable jurisdiction identity/status rows and the rights-cleared canonical selections stored in the immutable Q1 vintage, limited to CIA Factbook, Wikidata, and World Bank. Every fact carries the vintage label, cutoff, selected source-row ID, content hash, published method, and embedded source-rights record. Index, Pulse, alternates, restricted sources, images, constitution text, and raw publisher payloads are excluded. The legacy per-country mixed-source route stays blocked for DAT-027.
