...
### APR-D078 — Freedom House total score cannot substitute for the Index's PR+CL ratings input

**Decision:** `ci-research-panel-2000-2024-v2` supersedes but does not mutate v1. It retains the other four source series and replaces `fh_total_score` with the exact `pr_cl_total` input used by K1, reacquired from the already captured publisher workbook at SHA-256 `d6ac861af6e7dcea7e870e39ddbcd2925730a653c1466f8992a7d0005f53be88`. The captured ratings series begins in 2006; 2000–2005 are typed structural gaps and are never converted from 0–100 totals. Candidate set, preregistration, and baseline releases advance to v2 before outcome analysis. All v1 artifacts remain immutable historical evidence.
**Why:** Freedom House's total score and its Political Rights plus Civil Liberties ratings are distinct fields with different scales and semantics. Treating them as interchangeable would test a different composite while claiming to harden the live one. The corrected release changes baseline and factor hashes even where coverage counts stay constant, proving that identity metadata must govern the calculation rather than a shared publisher name.
