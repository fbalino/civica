...
### APR-D050 — The first Atlas export is a frozen source-observation package

**Decision:** `civica-atlas-export/v1` publishes one gzip-compressed JSON package for release `atlas-2026-07-11`. It contains stable jurisdiction identity/status rows and active country-fact observations only from CIA Factbook, Wikidata, and World Bank, the three sources represented in the live fact table whose public bulk-export terms are verified. Every fact joins to an embedded source-rights record. The package includes a field-level codebook, join contract, deterministic ordering, schema version, release/generated dates, counts, and a content hash. Index, Pulse, restricted or pending sources, images, constitution text, and raw publisher payloads are excluded. The legacy per-country mixed-source route stays blocked for DAT-027.
**Why:** Publishing the existing convenience dump would mix unresolved fields and research outputs with source rows that lack export clearance. A narrow frozen observation release gives researchers useful Atlas data now while keeping source terms inspectable and leaving canonical-plus-alternates export design to its dedicated task.
...
**Decision:** `civica-clean-room-fixture/v1` is a legally shareable miniature release input containing three jurisdictions and one permitted observation each from CIA Factbook, Wikidata, and World Bank. It runs through the production Atlas export builder, which fails on blocked source rights and missing jurisdiction joins. The checked contract requires exact fixture and canonical-export SHA-256 values, exact row counts, no runtime network request, and no database or model credential. The documented fresh-checkout proof installs only from the public lockfile and runs the strict reproduction, all repository tests, every validator, and the production build without private branches or copied caches. This fixture does not claim to reproduce Atlas source records whose original release inputs were never captured.
...
### APR-D055 — G2 reproduces the released observation package, not lost upstream bytes

**Decision:** `atlas-2026-07-11-g2-rc1` treats the normalized, rights-cleared source-observation rows as the frozen reproduction input for the Atlas release candidate. The bundle records their per-source semantic hashes, vintages, retrieval cuts, rights, and exact export-builder source, then reconstructs the canonical export and verifies both semantic and compressed hashes offline. It states that upstream publisher bytes which were not retained at release time cannot be replayed and are not represented as captured raw inputs. The G2 checklist passes for the frozen Atlas observation release; it does not certify end-to-end historical publisher ingestion, Index/Pulse replication, DOI registration, or source accuracy.
**Why:** Retrospectively inventing raw input hashes or acquisition times would corrupt provenance. Refusing to package anything would also discard a reproducible, legally distributable research artifact. The observation-level boundary makes the actual guarantee useful and testable while keeping the missing upstream-capture guarantee visible to reviewers.
