...
**Decision:** Adopt `pulse-evaluation-sampling-frame/v1` before gold-label access. Code the complete fixed-period event-candidate census, a stratified system-negative/unresolved sample, and a stratified sovereign-country-day retrieval sample. The two probability frames require 482 valid cases and an initial draw of 536 each, based on a conservative five-percentage-point precision target, planning design effect, and unusable-evidence allowance. Primary-stratum inclusion probabilities determine weights. Geography, time, language, source type, regime, and retained media-evidence environments are balancing margins. Famous historical cases remain regression-only.
