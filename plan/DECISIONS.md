...
### APR-D133 — One named human holds current publication authority

**Decision:** Adopt `civica-research-publication-governance/v1`. Fernando Baliño is the current accountable human for data, methodology, editorial copy, corrections, releases, security, source rights, reviewer-independence process, conflicts, and emergency action. Each domain has required evidence and a fail-closed condition. Agents and models may prepare and test work but cannot author, approve, publish, spend, waive conflicts, select or contact reviewers, accept risk, or restore a suspended surface. Future delegation requires a named, scoped, dated appointment. Independent reviewers own their conclusions; original reports remain separate from the author's response.

**Why:** A one-person project should disclose its concentration instead of implying an editorial body that does not exist. Naming the decision-maker makes responsibility inspectable. Frozen gates, records, and independent criticism limit unilateral error, while the charter admits that they do not create internal separation of duties. An unmanageable owner conflict therefore blocks the affected claim until a qualified independent person is formally appointed for that decision.

### APR-D134 — Personal authorship and organizational publishing stay distinct

**Decision:** Adopt `civica-authorship-contributions/v1`. Fernando Baliño is the responsible human author for the current project and frozen research releases, with the status “Independent project; no institutional affiliation claimed,” a public project contact route, nine declared contribution roles, and a dated contribution history linked to Git and plan evidence. No ORCID is recorded because none was supplied and a public search found no reliable match. Civica Atlas remains the organizational publisher for discovery and continuity. Canonical citation and release files must name the human author; organization-only authorship requires a versioned exception and none exists. The 15 generic blog bylines now name Fernando.

**Why:** A project label helps people find and cite the work, but it cannot accept responsibility, disclose a conflict, answer a correction, or consent to authorship. Separating human author from organizational publisher preserves the useful Civica identity while making accountability real. The declared roles describe responsibility without pretending that agents and tools were absent; GOV-004 records that assistance separately.

### APR-D135 — Model assistance is disclosed by role and cannot become peer review

**Decision:** Adopt `civica-ai-use-disclosure/v1` with eight material use classes: code/planning, internal agent audits, Pulse production classification, annotation/coding dry runs, structured extraction and summaries, reader chat, editorial prose, and editorial illustrations. The record names exact production systems where the repository preserves them, leaves incomplete historical session and image-generation metadata unknown, and states the control and limitation for each role. Fernando Baliño remains the responsible human. Models and agents cannot be authors, accountable decision-makers, independent peer reviewers, or gold-label substitutes.

**Why:** A generic “AI-assisted” label hides the difference between a drafting tool, a production classifier, a reader chatbot, an image generator, and an internal audit. Readers need to know which outputs can enter data or public copy, how those outputs are checked, and what remains unevaluated. Exact role-level disclosure also prevents agent agreement from being promoted into academic validation.

### APR-D136 — Frozen release authority preserves identity and appeal

**Decision:** Adopt `civica-release-correction-authority/v1`. Fernando Baliño approves release states, methodology versions, corrections, retractions, supersessions, emergency suppression, and restoration after applicable gates pass. Major, minor, patch, and beta meanings are fixed. Emergency action contains credible misinformation, security/privacy exposure, rights breaches, or unsafe automation while preserving lawful evidence. Corrected frozen bytes receive a new version and DOI with `IsNewVersionOf`/`IsPreviousVersionOf`; a retracted DOI resolves to a tombstone and is never reassigned. Notices span policy, corrections, artifact, DOI, and API surfaces. One evidence-bearing reconsideration is available, with independent adjudication when the original decision-maker has a material conflict.

**Why:** Correction policy without named authority or release identity can still permit silent overwrite. Frozen-object and DOI rules make the historical record durable, while the appeal route gives reporters a defined way to challenge process or present new evidence. Three no-write tabletop incidents prove that material error, methodology failure, and restricted-payload exposure produce distinct containment, version, DOI, notice, and appeal artifacts.

### APR-D137 — Advisory service is bounded and cannot imply endorsement

**Decision:** Adopt `civica-advisory-board-charter/v1`. The board advises on Atlas stewardship, governance measurement, political event data, reproducibility, accessibility, and source rights. It has no publication veto and does not replace Fernando Baliño as accountable decision-maker. Appointments run for 24 months with one possible renewal and approximately 8–16 hours of ordinary annual service. Standing service is unpaid; substantial reviews are separate, optional, and may receive an owner-approved outcome-independent honorarium. Conflicts may be managed, require recusal, or end participation. Names, affiliations, quotations, and review publication require specific consent. Board service validates neither Civica as a whole nor any particular claim, method, release, or later change.
