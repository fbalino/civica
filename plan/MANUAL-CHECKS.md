...
- **EXP-038 · Engineering reconciliation — RESOLVED 2026-09-17:** A4,
  H5, and T3 retain their recorded owner decisions. T4 now points Contact and
  the current project disclosure to the already-active `/report-data-issue`
  flow. Claims, disclosure/packet contracts, the canonical production build,
  and a real-browser check passed without submitting a report. Historical
  disclosure v1 and frozen July artifacts remain unchanged; current v2 is
  bound to the active September packet. Evidence: `plan/evidence/EXP-038/`.
  Deployment acceptance is recorded separately from local browser evidence.
...
- **PLT-001 · Repository enforcement — RESOLVED 2026-09-17:** the active default-branch ruleset now requires pull requests, `verify`, Vercel, current-base checks, resolved conversations, and squash merges; force pushes/deletion are blocked and no bypass actors are configured. PR #35 merged through this path after both required checks passed. Settings evidence: `plan/evidence/PLT-031/delivery-controls-2026-09-17.json`.
...
- **PLT-010 · Deployed non-model smoke completed; classification binding remains separate:** September 17 production verification used authenticated GET dry runs with stable keys. The Canadian repeat returned `200 duplicate_suppressed`; the retained successful execution has one attempt and source freshness is unchanged. Evidence: `plan/evidence/PLT-031/production-verification-2026-09-17.json`. The optional classifier remains intentionally locked; after an independently authorized ordinary classify run, verify its immutable delivery binding read-only. Do not start a paid manual classifier run solely for this check.
...
- **QA-019 · Owner/provider — external record and disposition pending:** use
  ...
  technical bad-Preview → forward-fix packet with a date. Do not claim the
  local changelog is an external status publication.
- **ATL-026 / ATL-027 · Owner/platform — production authority pending:** the disposable-child rehearsal now proves the component ledger, immutable release, exact replay, and changed-input refusal. If Fernando authorizes production, apply the complete checked authoritative migration tail in the documented order, publish a separately named immutable Conditions release from retained inputs, run the task-specific live validators, and retain decomposed rows/reference sets/public-read evidence. Staging evidence does not itself grant production authority.
- **PLT-019 · Owner/platform — production follow-up pending:** the retained
  exact-candidate isolated rehearsal is complete through
  `0051_eminent_jocasta`. Treat any production migration, Conditions
  publication, or deployment as a separate action following
  `data/DEPLOYMENT-REHEARSAL.md`; the staging packet is not production
  evidence.
...
- **ATL-010 · Owner/platform — RESOLVED 2026-08-09/10:** the owner-authorized
  production `sync:wikidata` run executed under named release
  `atlas-wikidata-refresh-20260809-v1` (authority
  `plan/evidence/QA-018/OWNER-SIGNOFF-2026-08-09.md`). The recaptured audit
  reports 0 roster discrepancies with `releaseReady=true`; Samoa's head of
...
- **PLT-007 · Owner — RESOLVED 2026-07-29:** the historically exposed Neon `neondb_owner` credential was reset independently on production main and the retained recovery branch. Fresh old pooled/direct authentication attempts are rejected; fresh replacement owner connections work and preserve future migration authority. The deployed `civica_app_20260727` role and Vercel environment were untouched, and the local owner environment is mode `0600`. Redacted evidence: `plan/evidence/PLT-007/owner-credential-rotation-2026-07-29.v1.json`. The historical bytes still exist in Git, so their non-reversible hash remains registered as `ROTATED/INERT`; a shared-history purge remains Fernando's separate decision.
