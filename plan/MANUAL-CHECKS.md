...
- **QA-018 · Owner/platform — new isolated run required:** the retained `0050` Preview packet remains valid historical evidence, but the integrated candidate adds forward migration `0051_eminent_jocasta`. Authorize a new disposable nonproduction run for the exact candidate through `0051`, retain the bounded checks, then record a dated approval or rejection. The earlier run does not authorize production or prove the new head.
...
- **PLT-019 · Owner/platform — production follow-up pending:** the retained isolated rehearsal is complete through `0050_index_release_header_contract`; the new `0051_eminent_jocasta` head still requires its own exact-candidate nonproduction run. Treat any production migration, Conditions publication, or deployment as a separate authorized action following `data/DEPLOYMENT-REHEARSAL.md`; no production promotion is claimed by the historical staging evidence.
