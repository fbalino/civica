...
- **PLT-001 · Owner/platform — hosted runs observed; repository enforcement pending:** September 17 pull-request and main `verify` runs passed (see `plan/evidence/PLT-001/README.md`). A read on that date still found main unprotected and no rulesets. Enable branch protection or a ruleset requiring `verify`, and retain the settings evidence. Do not repeat the completed CI implementation or claim enforcement from a passing run.
