...
- **QA-018 · Owner — RESOLVED 2026-08-09:** Fernando confirmed the isolated attempt-06 run in writing; the dated record is `plan/evidence/QA-018/OWNER-SIGNOFF-2026-08-09.md`. The same decision granted separate execution authority for the Conditions production batch, migrations 0046/0047, the Wikidata refresh + migration 0048, and the QA-019 rehearsal (each still runs under its own packet); the Pulse wave remains unauthorized pending an operating-cost decision.
...
- **PLT-019 · Owner/platform — production follow-up pending:** the isolated staging rehearsal is complete through `0050_index_release_header_contract` with a disposable Neon child and exact prebuilt Vercel Preview. Review the QA-018 packet, then treat any production migration, Conditions publication, or deployment as a separate authorized run following `data/DEPLOYMENT-REHEARSAL.md`; no production promotion is claimed by the staging evidence.
