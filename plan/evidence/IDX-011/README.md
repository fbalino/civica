...
Current release `civica-index-baselines/v3` reads panel v3 and keys every input by source plus indicator. Prior baseline releases remain preserved.
...
- B2 equal-weight common-scale mean: 2,270 complete four-dimension units
- B3 first common factor: 2,270 complete four-dimension units, fitted on 551 joint-development units only
...
The checked release at `data/releases/ci-index-baselines-v3/` contains coverage and cryptographic output/model hashes, not source values or country outputs. The private selection input has 24,250 source-indicator cells, of which 19,471 are observed. `npm run validate:index-tournament-baselines:live` recomputes B0–B3 and requires an exact byte-equivalent manifest.
