...
The archived **Civica Index** composite methodology uses {{state.civicaIndex.dimensionCount}} governance inputs and weights informed by a limited 2023 cross-section. It remains preserved research and is not Civica&rsquo;s selected public comparison product. The **Civica Pulse** includes {{state.pulse.backtest.cases.length}} hand-curated historical smoke-test scenarios, but the displayed run predates the current production ensemble and is not representative validation. **Reconciliation rules** are documented and live but the public-facing methodology page is being expanded as v1 closes out.
