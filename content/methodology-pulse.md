...
2. **Resolve incidents.** Normalize each new report under `{{ctx.clusterIdentityVersion}}`, embed it with `{{ctx.clusterEmbeddingModel}}`, and compare it with the incoming batch and recent persisted incidents inside a ±{{ctx.clusterWindowHours}}-hour window. The ingest-time country guess is diagnostic and does not partition candidates. An exact full normalized identity with compatible labels can merge automatically inside that window. A second automatic rule requires an exact normalized headline, the same resolved country and calendar date, and compatible labels. Semantic similarity of at least {{ctx.clusterSemanticThreshold}} with an identity anchor, or {{ctx.clusterSemanticOnlyThreshold}} without one, creates a review candidate. The embedding-unavailable fallback requires token similarity of at least {{ctx.clusterLexicalThreshold}} and anchor overlap of at least {{ctx.clusterLexicalAnchorThreshold}}; it also creates a review candidate and never an automatic merge. Stable incident IDs survive confirmed merges, while losing event projections and their source trails remain retained.
...
Classifier work is keyed to the exact method, prompts, voter/verifier/subject models, publication gate, decoding mode, and retry policy. Never-attempted clusters run before due retries. A failed attempt waits at least {{ctx.classificationInitialRetryMinutes}} minutes; later waits increase under the same bounded policy and cannot exceed {{ctx.classificationMaximumRetryHours}} hours. After {{ctx.classificationMaxAttempts}} unsuccessful attempts, the cluster enters `terminal_failure` and receives no further calls under that configuration. A valid `category="none"` result is a separate terminal outcome, not a failure or an observed zero. Every claim and completion phase is retained, and the cron result reports queue depth and the age of the oldest eligible cluster. Agreement is reconstructed from the stored classify runs. Each qualifying vote names its provider, model, prompt, method, complete configuration, and configured panel size; duplicate providers, mixed versions, and one-run results resolve to no ensemble agreement. 4. **Verify and attribute.** {{ctx.verifierProse}} makes a separate adversarial call against the majority verdict. The same model also participates as one voter, so this is a separate call rather than an independent model family. Jurisdiction attribution is another pass, currently {{ctx.subjectAttributorProse}}, run after classification. It receives human-readable names, ISO3 codes, and versioned aliases rather than an internal UUID alone; it records one primary jurisdiction plus any materially affected jurisdictions with separate rationales and evidence references. If no primary resolves, the attribution remains unresolved and the event cannot publish automatically. The ingest-time country remains only a compatibility projection. 5. **Weight.** Collapse likely republications and evidence from one publisher or underlying report, then count the remaining specialist and news evidence groups. Combine that count with the stored agreement label and the positive-event asymmetry rule. Information-environment context has no production multiplier. The resulting “corroboration weight” is a hand-set heuristic in [0, 1], not a calibrated probability of correctness and not a publication minimum. 6. **Review or publish.** {{ctx.reviewTiersProse}}, deadlocks/no quorum, and weak or degraded majorities paired with a verifier objection route to review. Automatic publication additionally requires a qualifying stored ensemble and resolved subject attribution. Single-engine and subscription-agent results always enter review. Retained automatic rows whose stored evidence cannot support their old agreement label are quarantined; human-reviewed publication remains distinct. Queued, quarantined, and rejected events do not affect public deltas. 7. **Score.** For published events in the trailing {{ctx.scoreWindowDays}}-day window, multiply severity by the heuristic weight and an absorption multiplier, apply category-specific exponential decay, sum by country and dimension, clamp to [{{ctx.deltaLowerBound}}, {{ctx.deltaUpperBound}}], and write API-only experimental deltas. The absorption multiplier can become zero only through a retained decision linking that exact event to a later comparable fixed-scale Index observation.

Human-review operations follow `{{ctx.reviewSlaVersion}}`: {{ctx.reviewSlaTargetsProse}}. These are internal monitoring thresholds, not staffed response guarantees or evidence that the classifications are valid. The queue records priority, age, escalation, deadline, bounded exceptions, and disposition. An exception explains a delay but never restores daily-completeness wording. Items that predate the contract remain unpublished in a separate legacy quarantine; they are not described as reviewed or rejected.
...
Raw items point to the ingest run that created them. Cluster and classification links are write-once. Events identify their classification, latest corroboration, and current publication decision runs; a human review has its own append-only audit row. Every dimensional computation writes one immutable output-history row per country and dimension before advancing the current projection, and both share the score run, as-of date, window, value, contributors, and derivation envelope. History, projection, and successful run completion commit in one transaction. A no-event projection is an internal zero tombstone with no contributors; the public API returns `null`, not a zero or stability claim. The decision ledger described below is also append-only.
...
`pulse-jurisdiction-attribution/v2` distinguishes the event's **primary** domestic subject from additional **affected** jurisdictions. The primary jurisdiction is the compatibility projection used by the current country ledger and experimental numeric path. Affected jurisdictions are descriptive: the event can appear in their country event API with its affected role, but it is not silently scored there as a domestic event.

Every new resolved attribution stores the exact `pulse-jurisdiction-entities/v1` catalog hash, `pulse-jurisdiction-aliases/v1`, canonical entity snapshot, role, rationale, and headline/description evidence reference. Unknown codes, malformed roles, supranational cases, or cases without a defensible primary fail closed. Retained one-country history is labeled as a legacy projection because its historical alias and entity inputs were not stored. Passing cross-border fixtures establishes contract behavior, not representative attribution accuracy.
...
Where the majority agrees on the category but differs on severity, the Pulse takes the majority tier and, on a tie, the **more severe** tier (the conservative reading); the published severity value is the median of the agreeing models. If one model errors or returns an unparseable answer, classification can degrade to the models that did respond. Each cluster/configuration state now records the actual configured voter and verifier set, attempts, retry schedule, terminal outcome, and any sanitized whole-attempt failure. Successful provider runs remain attached to the event. A degraded success still does not retain the failed provider's reason or a separate degradation flag.
...
Decay is exponential: `impact = severity × corroboration_weight × absorption_multiplier × exp(−ln2 × days / half_life)`.
...
The current Index registry contains several harmonized backcasts of the same 2024-Q4 observation period, not a sequential comparable release pair. No current event therefore qualifies as absorbed, and every current absorption multiplier is one. Conceptual overlap remains possible until a later fixed-scale release and explicit event-level link satisfy the gate.
