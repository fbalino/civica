...
### Worked example 8 — Marshall Islands population, dispute resolved by demotion

**Pattern.** The disputes system after resolution. A material-error gap — about 118% between the CIA's value and the World Bank's — was raised as a dispute rather than silently averaged, because the disagreement reflects a genuine definitional split, not a typo. Triage then resolved it by demoting rows out of the active set rather than leaving them to compete on freshness.

**Live rows.** The CIA reports 82,011 with a `(2024 est.)` stamp. The World Bank reports 37,548 (2024). UN WPP reports 37,548 (2024, bit-exact match). The IMF reports 33,000 (2031, projected). Wikidata holds an older 53,127 (2017) claim. When the material-error dispute was resolved, every row except the World Bank measurement was demoted out of the active set (`status='demoted'`), each stamped with the resolving dispute's id.

**Resolver outcome.** With only the World Bank's 37,548 (2024) left active, it is canonical via `decisionReason='single_source'`, and the fact is no longer flagged `(disputed)`. The CIA's divergent 82,011 estimate and the older and projected rows sit demoted — recorded for the provenance trail, but excluded from the candidate pool. The disputes log is at [/country/methodology/reconciliation/disputes](https://civicaatlas.org/country/methodology/reconciliation/disputes).

**Story.** The gap is a genuine multi-source disagreement, not a data-entry mistake. The CIA's 82,011 follows an accounting that counts the large Marshallese diaspora holding permanent right of residency under the Compact of Free Association with the United States; the World Bank and UN's 37,548 follows resident-population accounting. Both methodologies are defensible, which is why the disagreement surfaced as a dispute in the first place rather than being averaged away. The resolution keeps the resident-population measurement canonical and preserves the CIA reading demoted rather than deleted, so the full provenance trail — including the value not chosen — stays inspectable.
