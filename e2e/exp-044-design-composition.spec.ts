/** EXP-044 — confirmed composition-drift browser contract. */
...
const CAPTURE_DIR = process.env.EXP044_CAPTURE_DIR;
...
async function capture(
  ...
  await page.screenshot({
    path: path.join(CAPTURE_DIR, `${name}.png`),
    fullPage: false,
    // The developer-only typography lab is outside EXP-044's evidence scope.
    style: "[data-type-lab] { display: none !important; }",
  });
}
...
for (const viewport of [
  ...
  for (const theme of ["light", "dark"] as const satisfies Theme[]) {
    test(`/country is centered and sequential — ${viewport.name}/${theme}`, async ({
      ...
      await capture(page, `country-${viewport.name}-${theme}`);
      await page.locator("#country-letter-A").scrollIntoViewIfNeeded();
      await capture(page, `country-directory-${viewport.name}-${theme}`);
    });
  }
}

for (const theme of ["light", "dark"] as const satisfies Theme[]) {
  test(`Governance Change row is one hover surface — desktop/${theme}`, async ({
    page,
    errors,
  }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/governance-change", {
      waitUntil: "domcontentloaded",
    });
    await page.waitForLoadState("load", { timeout: 20_000 }).catch(() => {});
    await setTheme(page, theme);

    await expect(page.locator(".editorial-table-scroll")).toHaveCount(1);
    const row = page.locator(".editorial-data-table tbody tr").first();
    const cells = row.locator(":scope > th, :scope > td");
    await expect(row).toBeVisible();

    const before = await cells.evaluateAll((elements) =>
    ...
    expect(before[1].align).toBe("left");
    expect(before[1].weight).toBe("400");
    expect(new Set(before.map((cell) => cell.padding)).size).toBe(1);
    expect(new Set(before.map((cell) => cell.border)).size).toBe(1);

    await row.hover();
    const hoverBackgrounds = await cells.evaluateAll((elements) =>
    ...
    await capture(page, `governance-change-desktop-${theme}`);
  });
}
