...
export async function writeLegislatureComposition(db: Db, input: { bodyId: string; parties: PartyCompositionRow[]; sourceId: string; sourceUrl: string; sourceLicense: string; rawPayload: unknown }, options: { dryRun?: boolean; stampFreshness?: boolean; markSynced?: typeof markSourcesSynced } = {}) {
  ...
  if (!options.dryRun) {
    ...
    const existing = await db.select({ id: statements.id }).from(statements).where(and(eq(statements.subjectTable, "government_bodies"), eq(statements.subjectId, input.bodyId), eq(statements.predicate, "seats_per_parties"), eq(statements.sourceId, input.sourceId))).limit(1);
    ...
    else await db.insert(statements).values({ subjectTable: "government_bodies", subjectId: input.bodyId, predicate: "seats_per_parties", ...value });
  }
