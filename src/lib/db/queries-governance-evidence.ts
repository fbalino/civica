...
import { GOVERNANCE_EVIDENCE_INDICATORS, GOVERNANCE_EVIDENCE_RELEASE_ID, GOVERNANCE_EVIDENCE_YEAR, governanceEvidenceMeta, governanceEvidenceRights, type GovernanceEvidenceRow } from "@/lib/ci/governance-evidence";
...
export async function getGovernanceEvidence(slug: string) {
  ...
  const rows = raw.flatMap((row): GovernanceEvidenceRow[] => {
    ...
    return [{ ...row, label: meta.label, construct: meta.construct, direction: meta.direction, sourceUrl: meta.sourceUrl, lastSyncAt: row.lastSyncAt?.toISOString() ?? null, ...rights }];
  });
  ...
  return { country, year: GOVERNANCE_EVIDENCE_YEAR, releaseId: GOVERNANCE_EVIDENCE_RELEASE_ID, rows };
}
