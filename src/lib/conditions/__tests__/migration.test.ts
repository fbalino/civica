...
test("ATL-026/ATL-027 authoritative migrations apply the decomposable and frozen Conditions ledgers", async () => {
  ...
  try {
    ...
    const directionMigration = readFileSync(
      "drizzle/authoritative/0051_eminent_jocasta.sql",
      "utf8",
    ).replaceAll("--> statement-breakpoint", "");
    await database.exec(directionMigration);
    ...
    const [directionConstraint] = await database
      .query<{ definition: string }>(`
        SELECT pg_get_constraintdef(oid) AS definition
        FROM pg_constraint
        WHERE conname = 'conditions_normalization_parameter_shape_check'
      `)
      .then((result) => result.rows);
    assert.match(directionConstraint?.definition ?? "", /not_ranked/);
  } finally {
