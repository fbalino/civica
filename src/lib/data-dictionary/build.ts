...
const FIELD_DEFINITIONS: Readonly<Record<string, string>> = {
  ...
  source_id: "Identifier of the registered source responsible for this observation or record.",
  source_url: "Direct upstream URL captured for this record when available.",
  source_hash: "Stable hash of the captured upstream payload/value used for change detection and replay.",
  ...
  version_key: "Content-addressed identifier for the immutable Pulse stage-version envelope.",
  versions: "Structured Pulse stage identity covering method, ontology, pipeline, algorithm, prompt, provider/model set, source basket, and upstream runs.",
  ingest_run_id: "Immutable Pulse ingest-stage run that created this raw item.",
  cluster_run_id: "Write-once Pulse cluster-stage run that assigned this raw item to a cluster.",
  classification_run_id: "Immutable Pulse classification-stage run that produced or rejected this event candidate.",
  corroboration_run_id: "Latest versioned Pulse corroboration-stage run applied to this event.",
  publication_run_id: "Pulse classification or review run responsible for the event's current publication state.",
  computation_run_id: "Pulse score-stage run responsible for the stored computed output.",
  run_id: "Immutable Pulse pipeline-stage run linked to this audit action.",
  counts: "Structured terminal row and outcome counts recorded for the pipeline run.",
  ...
  upstream_vintage_label: "Publisher-supplied release or vintage label retained verbatim.",
  as_of: "Upstream effective or observation date for the value.",
  status: "Current workflow or record status under this table's declared contract.",
  ...
  license: "Declared source license label; authoritative reuse posture is resolved through the rights manifest.",
  ...
  fact_value_numeric: "Numeric representation of the source observation when the concept is numeric.",
  ...
  effective_from: "Beginning of the record or methodology's effective interval.",
  ...
  content_hash: "Hash of the captured content used for deduplication and replay identity.",
  raw_payload: "Structured source payload retained for audit or replay subject to source rights.",
...
function unitFor(name: string, sqlType: string): string {
  ...
  if (name.includes("percent") || name.endsWith("_pct")) return "percentage points (0–100 unless source documentation states otherwise)";
  if (name.includes("probability") || name.includes("confidence") || name.includes("score") || name.includes("index") || name.includes("weight") || name.includes("delta")) {
    return "dimensionless; scale defined by the linked methodology or source";
  }
  if (name.endsWith("_year") || name === "year" || name.endsWith("_year_updated")) return "Gregorian calendar year";
  if (sqlType === "date") return "ISO 8601 calendar date";
  if (sqlType.startsWith("timestamp")) return "UTC timestamp (database type currently omits timezone)";
  if (name.includes("duration") || name.endsWith("_ms")) return name.endsWith("_ms") ? "milliseconds" : "duration in the unit declared by the producer";
  if (/(^|_)(count|seats|votes|population|total|rank|order|limit)$/.test(name) || /_(count|seats|votes|population|total|rank|order)$/.test(name)) return "count";
...
function temporalSemantics(name: string, policy: TablePolicy): string {
  if (name === "created_at") return "Civica row-creation time; not an observation or publisher vintage.";
  if (name === "updated_at") return "Civica row-update time; not an observation or publisher vintage.";
  if (name === "retrieved_at" || name === "fetched_at" || name === "ingested_at" || name === "last_synced_at" || name === "last_sync_at") {
    return "Civica retrieval/processing time; distinct from source observation and release vintage.";
  }
  if (name.includes("methodology_version") || name === "method_version" || name === "version") {
    return "Interpretation/version identifier; not a source observation date.";
  }
  if (name.includes("vintage") || name.endsWith("_year") || name === "year" || name === "as_of" || name.includes("date") || name.endsWith("_at")) {
    return `${capitalize(humanize(name))} follows this table contract. ${policy.vintageSemantics}`;
  }
  return policy.vintageSemantics;
}

function sourceFor(name: string, policy: TablePolicy, key: DictionaryKey): string {
  if (key.primary && name === "id") return "Civica-generated database identifier.";
  if (key.foreignKey) return `Relational identifier referencing ${key.foreignKey}; substantive provenance follows this table's source contract.`;
  ...
  if (name.startsWith("source_") || name === "references" || name.startsWith("wikidata_")) {
    return `Captured source-lineage metadata. ${policy.sourceOrDerivation}`;
  }
  return policy.sourceOrDerivation;
}

function columnDeprecation(tableName: string, columnName: string, policy: TablePolicy): DeprecationStatus {
  if (policy.deprecation.status === "legacy") return policy.deprecation;
  if (tableName === "jurisdictions" && ["capital", "population", "gdp_billions", "area_sq_km", "languages", "currency", "democracy_index"].includes(columnName)) {
    return {
      status: "legacy",
...
export function buildSchemaDataDictionary(): SchemaDataDictionary {
  ...
  for (const [exportName, table] of Object.entries(dbSchema).filter(([, value]) => isTable(value))) {
    const config = getTableConfig(table as never) as unknown as RuntimeTableConfig;
    const policy = TABLE_POLICIES[config.name];
    if (!policy) throw new Error(`data dictionary missing table policy for ${config.name}`);
    ...
    for (const index of config.indexes) {
      ...
      for (const column of columnsInIndex) {
        ...
        indexed.set(column.name, [...(indexed.get(column.name) ?? []), index.config.name]);
        if (index.config.unique) unique.set(column.name, [...(unique.get(column.name) ?? []), index.config.name]);
      }
    }
    ...
    for (const foreignKey of config.foreignKeys) {
      ...
      reference.columns.forEach((column, index) => {
        foreign.set(column.name, `${foreignConfig.name}.${reference.foreignColumns[index]?.name ?? "id"}`);
      });
    }

    const propertyByColumn = new Map<string, string>();
    for (const [propertyName, column] of Object.entries(table as unknown as Record<string, unknown>)) {
    ...
    const columns = config.columns.map((column) => {
      const key: DictionaryKey = {
        ...
        unique: Boolean(
          column.isUnique || individuallyUnique.has(column.name),
        ),
  ...
  const orphanPolicies = Object.keys(TABLE_POLICIES).filter((name) => !seenPolicies.has(name));
  if (orphanPolicies.length) {
    throw new Error(`data dictionary policies do not map to schema tables: ${orphanPolicies.join(", ")}`);
  }

  tables.sort((a, b) => a.name.localeCompare(b.name));
  for (const table of tables) table.columns.sort((a, b) => a.name.localeCompare(b.name));
  ...
  const count = (scope: TablePolicy["releaseScope"]) => tables.filter((table) => table.releaseScope === scope).length;
  return {
...
export function dictionaryValidationErrors(dictionary: SchemaDataDictionary): string[] {
  const errors: string[] = [];
  if (dictionary.summary.tables !== dictionary.tables.length) errors.push("summary table count does not match table inventory");
  const columns = dictionary.tables.flatMap((table) => table.columns);
  if (dictionary.summary.columns !== columns.length) errors.push("summary column count does not match column inventory");
  for (const table of dictionary.tables) {
    for (const field of ["definition", "rowGrain", "sourceOrDerivation", "cadence", "vintageSemantics", "rights"] as const) {
      if (!table[field]?.trim()) errors.push(`${table.name} is missing ${field}`);
    }
    if (!table.columns.length) errors.push(`${table.name} has no documented columns`);
    for (const column of table.columns) {
      for (const field of ["definition", "sqlType", "nullableMeaning", "unit", "sourceOrDerivation", "cadence", "vintageSemantics", "rights"] as const) {
        if (!String(column[field] ?? "").trim()) errors.push(`${table.name}.${column.name} is missing ${field}`);
      }
      if (/\b(TODO|TBD)\b/i.test(JSON.stringify(column))) errors.push(`${table.name}.${column.name} contains placeholder metadata`);
    }
  }
  return errors;
}

export function canonicalDictionaryJson(dictionary: SchemaDataDictionary): string {
  return `${JSON.stringify(dictionary, null, 2)}\n`;
}

// Retained as a small pure seam for seeded drift tests.
export function sameDictionary(left: SchemaDataDictionary, right: SchemaDataDictionary): boolean {
