...
const topLevelRows: AtlasSurfaceMatrixRow[] = [
  ...
  row({
    ...
    storage: ["elections", "jurisdictions", "sources"],
    fields: [
      "type",
      "date",
      "status",
    ...
    provenance: [
      "Election rows carry source_id/source_url; the page reads source freshness.",
    ],
    coverage: [
      "Upcoming and recent-result slices plus aggregate geography/type counts.",
    ],
    states: serverStates({
      empty:
        "Empty arrays render the client explorer with no-result states; query failure behavior is route-level.",
    }),
    tests: ["src/lib/elections/__tests__/writer-repeatability.test.ts"],
    testGap: null,
