...
import { syncOnsUk } from "../sync-ons-uk";

const jurisdiction = { id: "11111111-1111-4111-8111-111111111111", slug: "united-kingdom", iso2: "GB", iso3: "GBR" };
const latest: OnsYearPoint = { year: 2025, value: 69000000, updateDate: "2026-01-01" };
...
const fetchIndicator = async () => ({ latest, observationCount: 1, rejectedNoValue: 0, upstreamReleaseDate: "2026-01-01" });
...
test("ONS fixture applications converge on one canonical fact", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, ensureSource: (async () => false) as never, fetchIndicator: fetchIndicator as never, persistDisputes: noDisputes as never, markSynced: (async () => ["ons_uk"]) as never };
...
test("ONS dry-run is stable and performs zero database writes", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, ensureSource: (async () => false) as never, fetchIndicator: fetchIndicator as never, persistDisputes: noDisputes as never, markSynced: (async () => []) as never, dryRun: true };
