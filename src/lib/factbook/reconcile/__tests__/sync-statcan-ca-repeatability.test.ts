...
import { syncStatCanCa } from "../sync-statcan-ca";

const jurisdiction = { id: "11111111-1111-4111-8111-111111111111", slug: "canada", iso2: "CA", iso3: "CAN" };
const observation: StatCanObservation = { refPer: "2026-Q1", refPerRaw: "2026-01-01", value: 41472081, releaseTime: "2026-04-01", frequencyCode: 12 };
...
test("StatCan fixture applications converge on one canonical fact", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, fetchObservations: async () => [observation], persistDisputes: noDisputes as never, markSynced: (async () => ["statcan_ca"]) as never };
...
test("StatCan dry-run is stable and performs zero database writes", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, fetchObservations: async () => [observation], persistDisputes: noDisputes as never, markSynced: (async () => []) as never, dryRun: true };
...
test("StatCan upstream failure cannot stamp freshness", async () => {
  ...
  const result = await syncStatCanCa(state.db, {
    ...
    jurisdiction,
    fetchObservations: async () => { throw new Error("upstream schema changed"); },
