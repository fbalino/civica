...
test("IBGE fixture applications converge on one canonical fact", async () => {
  const state = harness();
  const options = { factKey: "population_total", tableId: "6579", jurisdiction, fetchIndicator: fetchIndicator as never, persistDisputes: noDisputes as never, markSynced: (async () => ["ibge_br"]) as never, atlasReleaseId: "atlas-test", writeFact: state.writeFact };
