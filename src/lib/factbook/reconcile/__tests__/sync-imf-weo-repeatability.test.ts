...
function harness() {
  ...
  const db = {
    ...
    select: () => ({
      from: (table: unknown) => ({
        where: () => ({ limit: async () => table === factSnapshots ? [{ id: [...snapshots.values()][0]?.id }] : [] }),
      }),
    }),
  };
...
test("IMF WEO fixture applications converge on one canonical fact", async () => {
  ...
  const options = {
    ...
    fetchCatalog: catalog as never,
    fetchIndicator: async () => ({ FIX: { "2025": 2.5 } }),
    persistDisputes: noDisputes as never,
    markSynced: (async () => ["imf_weo"]) as never,
  };
  await syncImfWeo(state.db, options);
...
test("IMF WEO dry-run is stable and performs zero database writes", async () => {
  const state = harness();
  const options = {
    factKey: "inflation_rate",
    weoCode: "PCPIPCH",
  ...
  const first = await syncImfWeo(state.db, options);
  const second = await syncImfWeo(state.db, options);
  assert.deepEqual(first.countersByFactKey, second.countersByFactKey);
  assert.equal(state.writes(), 0);
});

test("IMF WEO upstream failure cannot stamp freshness", async () => {
  const state = harness();
