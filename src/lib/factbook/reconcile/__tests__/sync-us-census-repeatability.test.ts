...
import { syncUsCensus } from "../sync-us-census";

const jurisdiction = { id: "11111111-1111-4111-8111-111111111111", slug: "united-states", iso2: "US", iso3: "USA" };
...
test("US Census fixture applications converge on one canonical fact", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, fetchValues: async () => [340000000], persistDisputes: noDisputes as never, markSynced: (async () => ["us_census"]) as never };
...
test("US Census dry-run is stable and performs zero database writes", async () => {
  const state = harness();
  const options = { factKey: "population_total", jurisdiction, fetchValues: async () => [340000000], persistDisputes: noDisputes as never, markSynced: (async () => []) as never, dryRun: true };
...
test("US Census upstream failure cannot stamp freshness", async () => {
  ...
  const result = await syncUsCensus(state.db, {
    ...
    jurisdiction,
    fetchValues: async () => { throw new Error("upstream schema changed"); },
