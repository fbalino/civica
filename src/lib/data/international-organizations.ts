...
export const ORGANIZATIONS: Organization[] = [
  ...
  },
  {
    ...
    memberCount: 15,
    description:
      "15-member Council responsible for international peace and security.",
    extra: { seats: 15 },
  },
  {
    id: "who",
  ...
  },
  {
    ...
    memberCount: 178,
    description:
      "UN-affiliated agency promoting peaceful use of nuclear energy and inhibiting its weaponization.",
  },
...
function orgMemberEntries(
  ...
  return entries.map(
    ([countryId, joinYear = DEFAULT_JOIN_YEAR, role = null]) => ({
      orgId,
      countryId,
      joinYear,
      role,
...
export const MEMBERSHIPS: Membership[] = [
  ...
  ...[
    ...
    orgId: "wto",
    countryId: c,
    joinYear: c === "chn" ? 2001 : c === "sau" ? 2005 : 1995,
    role: null as MemberRole,
  })),
  ...
  ...[
    ...
    orgId: "oecd",
    countryId: c as string,
    joinYear: y as number,
...
export function getCoMembers(
  countryId: string,
): Array<{ countryId: string; sharedCount: number }> {
  const myOrgs = new Set(
    MEMBERSHIPS.filter(
      (m) => m.countryId === countryId && m.status !== "withdrawn",
    ).map((m) => m.orgId),
  );
  const counts = new Map<string, number>();
  for (const m of MEMBERSHIPS) {
    if (m.countryId === countryId || m.status === "withdrawn") continue;
