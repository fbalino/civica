...
export async function writeGovernmentTaxonomies(db: Db, rows: GovernmentTaxonomyInput[], options: { dryRun?: boolean; sourceId?: string; markSynced?: typeof markSourcesSynced } = {}) {
  ...
  const keys = new Set<string>(); for (const row of rows) { const key=`${row.jurisdictionId}:${row.taxonomyVersion}`; if(keys.has(key)) throw new Error(`Duplicate government taxonomy: ${key}`); keys.add(key); if(!row.derivationVersionKey || !row.derivationVersions) throw new Error(`Missing derivation version: ${key}`); }
