...
export async function writeElection(
  ...
  if (
    !input.election.jurisdictionId ||
    !input.election.electionDate ||
    !input.election.electionType
  )
  ...
  if (input.election.wikidataQid) {
    const byQid = await db
      .select({ id: elections.id })
      .from(elections)
      .where(eq(elections.wikidataQid, input.election.wikidataQid))
      .limit(1);
    match = byQid[0]?.id ?? null;
  }
  if (!match) {
    const bodyIdentity = input.election.bodyId
      ? eq(elections.bodyId, input.election.bodyId)
      : isNull(elections.bodyId);
    const byContest = await db
      .select({ id: elections.id })
      .from(elections)
      .where(
        and(
          eq(elections.jurisdictionId, input.election.jurisdictionId),
          bodyIdentity,
          eq(elections.electionDate, input.election.electionDate),
          ilike(elections.electionType, input.election.electionType),
        ),
      )
      .limit(1);
    match = byContest[0]?.id ?? null;
  }
  ...
  if (match) {
    electionId = match;
    await db.update(elections).set(input.election).where(eq(elections.id, electionId));
    updated = 1;
  } else {
    const row = await db
      .insert(elections)
  ...
  if (input.results) {
    ...
    for (const result of input.results)
      await db.insert(electionResults).values({ electionId, ...result });
  }
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(statements.subjectId, electionId),
        eq(statements.predicate, input.provenance.predicate),
        eq(statements.sourceId, input.provenance.sourceId),
      ),
    )
    .limit(1);
  const value = {
    objectValue: input.provenance.objectValue,
    sourceId: input.provenance.sourceId,
    sourceUrl: input.provenance.sourceUrl,
    sourceLicense: input.provenance.sourceLicense,
  ...
  if (existing[0])
    ...
    await db.insert(statements).values({
      ...
      predicate: input.provenance.predicate,
      ...value,
    });
  return {
    inserted,
