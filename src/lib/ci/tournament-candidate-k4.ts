...
export interface K4PairingOutput {
  candidateId: "K4"; unitId: string; jurisdictionId: string; iso3: string; constructId: string; practiceYear: number;
...
export function runK4PairingPrototype(excerpts: readonly K4ExcerptInput[], practice: readonly K4PracticeInput[]): K4PairingOutput[] {
  ...
  return jurisdictions.flatMap(([jurisdictionId, iso3]) => K4_PRACTICE_INDICATORS.map((indicator) => {
    ...
    return {
      candidateId: "K4" as const, unitId: `${iso3}:${indicator.constructId}:${K4_PAIRING_YEAR}`, jurisdictionId, iso3, constructId: indicator.constructId, practiceYear: K4_PAIRING_YEAR,
