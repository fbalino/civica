...
export const TOURNAMENT_EVALUATION_INTERFACE_VERSION = "civica-tournament-evaluation-interface/v1" as const;
export type TournamentSplitCoverage = { development: number; validation: number; finalHoldout: number };
export type TournamentEvaluationArtifact = {
  artifactId: "K0" | "K1" | "K2" | "K3" | "K4" | "K5";
  methodVersion: string; outputKind: "native_dashboard" | "score_estimate" | "concordance_profile" | "ledger_state" | "evidence_pair" | "relation_candidate";
  unit: string; inputReleaseIds: readonly string[]; possibleUnits: number | null; emittedUnits: number; withheldOrMissingUnits: number | null;
  splitCoverage: TournamentSplitCoverage; evidenceCoverage: Readonly<Record<string, number>>;
  uncertainty: { mode: "publisher_bounds" | "not_retained" | "not_estimable" | "categorical_state" | "pending_coder_reliability"; boundedUnits: number; note: string };
  outputSha256: string; valuesLocation: string; publicValuesIncluded: false;
  heldoutLabelAccess: "not_applicable_no_labels_used" | "sealed_not_inspected";
...
export function buildTournamentEvaluationSuite(input: { baselines: BaselinesInput; k1: K1Input; k2: K2Input; k3: K3Input; k4: K4Input; k5: K5Input }) {
  ...
  return { schemaVersion: "civica-tournament-evaluation-suite/v1", interfaceVersion: TOURNAMENT_EVALUATION_INTERFACE_VERSION, protocolVersion: input.k1.protocolVersion, artifacts, suiteSha256: researchPanelHash(artifacts), heldoutOutcomeMetricsComputed: false, winnerSelected: false };
}

export function tournamentEvaluationErrors(suite: ReturnType<typeof buildTournamentEvaluationSuite>): string[] { const errors: string[] = []; const ids = suite.artifacts.map((row) => row.artifactId); if (ids.join(",") !== "K0,K1,K2,K3,K4,K5") errors.push("candidate suite is incomplete or unordered"); for (const row of suite.artifacts) { if (!/^[a-f0-9]{64}$/.test(row.outputSha256)) errors.push(`${row.artifactId} output hash invalid`); const splitTotal = row.splitCoverage.development + row.splitCoverage.validation + row.splitCoverage.finalHoldout; if (splitTotal !== row.emittedUnits) errors.push(`${row.artifactId} split coverage does not equal emitted units`); if (row.possibleUnits !== null && row.withheldOrMissingUnits !== null && row.emittedUnits + row.withheldOrMissingUnits !== row.possibleUnits) errors.push(`${row.artifactId} possible-unit accounting does not close`); if (row.publicValuesIncluded !== false) errors.push(`${row.artifactId} leaks private values`); } if (suite.heldoutOutcomeMetricsComputed || suite.winnerSelected) errors.push("suite inspected holdout outcomes or selected a winner"); return errors; }
