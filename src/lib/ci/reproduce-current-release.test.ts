...
test("clean-room reproduction is byte-stable across repeated runs", () => assert.deepEqual(reproduceCurrentCiRelease(spine, inputs, 500), reproduceCurrentCiRelease(spine, inputs, 500)));
test("input order cannot change dimensions, simulation, or rank", () => assert.deepEqual(reproduceCurrentCiRelease(spine, [...inputs].reverse(), 500), reproduceCurrentCiRelease(spine, inputs, 500)));
