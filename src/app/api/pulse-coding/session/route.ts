...
export async function POST(request: NextRequest) {
  ...
  const result = await authenticatePulseCodingAccessCode(accessCode, randomUUID());
  if (!result) {
  ...
  for (const [name, value] of result.cookieHeaders) response.headers.append(name, value);
  return response;
}
