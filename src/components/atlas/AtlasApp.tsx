...
export default function AtlasApp({ dbCountries, dbChambers }: AtlasAppProps) {
  ...
  const worldMapRef = useRef<AtlasWorldMapHandle>(null);
  ...
  // Load TopoJSON world data. Kept at the AtlasApp level because the mini-map
  // inside the left rail (extracted in a follow-up commit) renders the same
  // paths.
  useEffect(() => {
    let cancelled = false;
    async function load() {
  ...
  function enterExplore(id: string) {
    ...
    setTab("chamber");
    setDimmed(new Set());
    worldMapRef.current?.flyTo(id);
    setTimeout(() => setMode("explore"), 500);
  }

  function handleCountrySelect(c: Country, { shift }: { shift: boolean }) {
  ...
  return (
    <div className="atlas-root" style={{ position: "fixed", top: navHeight, left: 0, right: 0, bottom: 0, zIndex: 30, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "relative", flex: 1, overflow: "hidden" }}>
        ...
        <div className="atlas-view" style={{ display: mode === "atlas" ? "block" : "none", position: "absolute", inset: 0 }}>
          <AtlasWorldMap
            ...
            mapPaths={mapPaths}
            mapLoaded={mapLoaded}
            filteredCountryIds={filteredCountryIds}
            pinned={pinned}
            onCountrySelect={handleCountrySelect}
            onUnpinAt={(i) => setPinned((prev) => prev.filter((_, j) => j !== i))}
            onOpenCompare={() => {
              if (pinned.length < 2) return;
              setCompareA(pinned[0]);
              setCompareB(pinned[1]);
              enterCompare();
            }}
          />
        </div>
      ...
      {/* Hover card now rendered inside AtlasWorldMap. */}
...
function OrgDetailPanel({
  ...
  loading: boolean;
  mapPaths: Array<{ d: string; id: string | null; country: Country | null; neId: string; centroid: [number, number]; area: number }>;
  mapLoaded: boolean;
  onPickCountry: (slug: string) => void;
}) {
  if (loading && !detail) {
  ...
  const sortedMembers = [...detail.members].sort((a, b) => (a.joinYear ?? 9999) - (b.joinYear ?? 9999));
  ...
  return (
    <>
      <div className="atlas-masthead">
        <div>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
            ...
            >
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: typeVar }} />
              {typeLabel}
            </span>
          </div>
        </div>
        <div className="quick-facts">
          <div className="r"><b>Members</b><span>{detail.members.length}</span></div>
          <div className="r"><b>Founding</b><span>{founding}</span></div>
          <div className="r"><b>Observers</b><span>{observers}</span></div>
          {o.foundedYear && <div className="r"><b>Founded</b><span>{o.foundedYear}</span></div>}
          {o.hqCountry && <div className="r"><b>HQ</b><span>{o.hqCountry.toUpperCase()}</span></div>}
        </div>
      </div>
      ...
      <div className="intl-section-head">Membership map <span>{detail.members.length} countries highlighted</span></div>
      <div className="intl-panel" style={{ padding: 8 }}>
        <svg viewBox="0 100 2000 800" preserveAspectRatio="xMidYMid meet" className="org-mini-map">
          {mapLoaded
            ? mapPaths.map((p, i) => {
      ...
      <div className="intl-section-head">Regional distribution <span>share of membership</span></div>
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${regionOrder.filter((r) => regionCounts.get(r)).length || 1}, 1fr)`, gap: 8, margin: "8px 0 4px" }}>
        {regionOrder.map((r) => {
          const ct = regionCounts.get(r);
          if (!ct) return null;
          const pct = Math.round((ct / totalMembers) * 100);
          return (
            <div key={r} style={{ border: "1px solid var(--atlas-rule-2)", padding: "10px 12px", background: "var(--atlas-paper-2)" }}>
              <div className="atlas-mono" style={{ fontSize: 10, color: "var(--atlas-muted)", letterSpacing: ".14em", textTransform: "uppercase" }}>{r}</div>
              <div className="atlas-serif" style={{ fontSize: 20, letterSpacing: "-0.01em", marginTop: 2 }}>{ct} <span style={{ fontSize: 10, color: "var(--atlas-muted)", letterSpacing: ".1em" }}>&middot; {pct}%</span></div>
              <div style={{ height: 3, background: "var(--atlas-rule)", marginTop: 6, position: "relative" }}>
                <div style={{ position: "absolute", inset: 0, width: `${pct}%`, background: typeVar }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="intl-section-head">Members <span>join year ascending</span></div>
          ...
          {sortedMembers.map((m) => {
            ...
            return (
              <div
                ...
                onClick={() => memberBySlug.has(m.slug) && onPickCountry(m.slug)}
              >
                <span className="dot" style={{ background: typeVar }} />
                <span className="name">
                  {m.name}
                  <span className="full">{m.region} &middot; {m.id.toUpperCase()}</span>
                </span>
                <span className="year">{m.joinYear ?? "\u2014"}</span>
                {m.role && <span className={badgeClass}>{isP5 ? "P5" : m.role}</span>}
              </div>
