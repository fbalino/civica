...
export function GovernanceEvidenceTable({ countryName, rows }: { countryName: string; rows: GovernanceEvidenceRow[] }) {
  return (
    <DataTable aria-label={`Source-native governance evidence for ${countryName}`}>
      <thead><tr><th>Source and measure</th><th>Native observation</th><th>Publisher uncertainty</th><th>Series, vintage, and access</th></tr></thead>
      <tbody>{rows.map((row) => (
        <tr key={`${row.sourceId}:${row.indicatorId}`}>
          ...
          <td><strong>{row.sourceVintage}</strong><br /><small>{row.seriesType === "harmonized_backcast" ? "Harmonized backcast" : "As-published release"}</small><br /><Link href={row.sourceUrl}>Publisher file</Link><br /><small>{row.exportPermission === "allowed" ? "Civica export allowed with attribution" : "Download from publisher; Civica bulk export blocked"}</small></td>
        </tr>
      ))}</tbody>
    </DataTable>
  );
