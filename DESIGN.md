...
- **Explore megamenu** (`.explore-menu` in `globals.css`; data in `src/components/exploreNavItems.ts`) — the header&rsquo;s single browse surface. One `Explore` trigger in the `.nav-dropdown` register (same chevron + CSS `:hover`/`:focus-within` reveal as the Index/Methodology dropdowns) drops a wide two-column panel: an ivory-paper surface with a terracotta hairline top rule, `--radius-lg`, and a `--shadow-hard-lg`. Each column has a small-caps Archivo label (`Countries & Places`, `Politics & Data`) and four destination rows; every row pairs a spot engraving (`public/engravings/spot-*.webp` + `-dark`, swapped via `theme-engraving-light/-dark`) with a serif name and a one-line Archivo description, warming on hover/focus (accent hairline + tint, engraving nudges, reduced-motion aware). The eight hrefs — `/country`, `/atlas`, `/compare`, `/constitution`, `/parties`, `/elections`, `/rankings`, `/organizations` — are the single source consumed by both the desktop `NavLinks` panel and the full-screen `MobileNav` browse register. Demoed on `/design-system`. Add or reorder surfaces in `exploreNavItems.ts`, never per-consumer.
