...
Use Newsreader for display and country/editorial headings through **`var(--font-heading)`**. (`--font-serif` is a deprecated alias preserved for backwards compatibility — new code uses `--font-heading`.) The production face carries the owner-approved `120%` calibration from the Type Lab. Hero and country-masthead titles use **`var(--font-weight-light)`**; H2 and lower editorial headings use **`var(--font-weight-regular)`**. The canonical page H1 is 56px (`var(--text-56)`) except for `/blog`, which is allowed to keep its separate editorial nameplate. Use Archivo for body/interface text through **`var(--font-body)`**; its production face carries the approved `106%` calibration. (`--font-body-sans` and `--font-sans` are deprecated aliases preserved for backwards compatibility — new code uses `--font-body`.)

**Monospace is reserved for literal code/API snippets only.** Use **`var(--font-code)`** for code blocks, curl examples, endpoint paths, and cssVar display chips. Do NOT use monospace for labels, IDs, source/meta rows, eyebrows, numeric UI, or any readable fact — those are Archivo. (The legacy `var(--font-mono)` token is now repointed to the Archivo stack as a safety net, so any straggler renders Archivo, not monospace.) Small-caps "eyebrow" labels use Archivo with `text-transform: uppercase` + letter-spacing — not a monospace font. For numeric columns that need to align (tables, scores, stat values), use Archivo with `font-variant-numeric: tabular-nums`.

Use `var(--text-*)` tokens for font sizes. Do not add new pixel font sizes in page-level CSS.

Font weight tokens: `--font-weight-light` (300), `--font-weight-regular` (400), `--font-weight-medium` (500), `--font-weight-semibold` (600), `--font-weight-bold` (700). (`--font-weight-mono` is a legacy alias of medium kept for backwards compatibility — new code uses the named weights directly.) Avoid hardcoded numeric `font-weight` values in new code.

The canonical leading tokens are the approved 120% Type Lab calibration: `--leading-none` (1.2), `--leading-tight` (1.26), `--leading-snug` (1.32), `--leading-normal` (1.98), `--leading-relaxed` (2.04), and `--leading-loose` (2.1). Components consume the role token rather than recreating a line-height.
...
- **Screen-reader-only text** — use the global `.sr-only` utility for a real
  accessible label or heading that should not occupy the visual canvas. Add
  `aria-hidden="true"` only when the content must also be removed from the
  accessibility tree, such as a form bot trap. Never recreate off-screen
  clipping inline or page-locally; `/design-system` exercises this shared
  utility through its accessible page title.
...
- `<CountryDirectory>` (`src/components/country/CountryDirectory.tsx`) — the canonical A–Z country directory shared by `/country`, `/governance-evidence`, and the `/design-system` demo. It owns alphabet grouping, hairline headings, and responsive columns; rows are single-line (fixed-width flag slot, serif name, ellipsis truncation) with a muted status suffix only for entries whose `jurisdiction-status/v1` label is not the "UN member state" default — the exceptional statuses stay visible while the baseline case carries no per-row noise (owner cleanup 2026-08-17; the trailing region dots are retired). Callers provide only their country data and destination (`hrefPrefix` plus optional `queryParam`). Never recreate a page-local country link grid.
- Search fields are **fully rounded** (`--radius-search`) with a leading magnifier + Archivo placeholder (see `CountrySearchCombobox` / `GlobalSearch`). This is the canonical shape for every search box site-wide (owner decision 2026-07-01).
- `DataTable`
...
- **Explore dropdown** (`ExploreMenuPanel`, rendered inside the shared `.nav-dropdown-menu`; data in `src/components/exploreNavItems.ts`) — one `Explore` disclosure opens a standard grouped header dropdown, visually identical to the Governance Evidence and Methodology menus (owner decision 2026-08-17, replacing the image-led EXP-015 megamenu: no photos, no descriptions, no panel header). Two `.nav-dropdown-group`s — `Start with a place` and `Research tools` — separated by the hairline group divider, each introduced by a `.nav-dropdown-group-label` and listing plain `.nav-dropdown-item` text links. The panel keeps the JS open state (`aria-expanded`, Escape-to-close, hover intent) and a short `@starting-style` fade. The eight hrefs — `/country`, `/atlas`, `/compare`, `/constitution`, `/parties`, `/elections`, `/rankings`, `/organizations` — are the single source consumed by both desktop `NavLinks` and the full-screen `MobileNav` browse register (mobile keeps each entry's one-line description, without artwork). Demoed on `/design-system`. Add or reorder surfaces in `exploreNavItems.ts`, never per-consumer, and render the desktop list through `ExploreMenuPanel` rather than recreating the composition.
- **Analytics consent bar** (`CookieConsentBanner` + `AnalyticsPreference`, `.cookie-consent*` / `.analytics-preference*` in `globals.css`) — the one canonical consent surface. A bottom-fixed, non-blocking bar at `--z-toast` on the elevated surface with a hairline border and `--shadow-hard-lg`; it stacks to a single column under 640px with full-width actions. Accept and decline are both real `<Button>`s of equal prominence (`primary` / `secondary`) — never a styled-down "reject" link, and never a modal that traps the reader, because an undecided reader already has analytics off. It renders only when `NEXT_PUBLIC_POSTHOG_KEY` is configured and the stored decision is `pending`. The standing reader control lives at `/privacy#analytics`; the decision contract is `src/lib/analytics/consent.ts`. Demoed on `/design-system`. Do not add a second cookie/consent surface, and do not gate content behind it.
- **Full-screen atlas menu** (`MobileNav`, `.mobile-menu*` in `globals.css`) — the hamburger opens a full-viewport, scroll-contained navigation surface rather than a side drawer. It uses the canonical hero engraving as a quiet full-canvas plate, the shared `EXPLORE_NAV_GROUPS` browse destinations as plain labelled text entries with their one-line descriptions (no per-destination artwork, per the 2026-08-17 owner decision), a separate research/methodology register, the global search slot, operational status, and reference/legal links. The modal locks body scroll, focuses Close on entry, traps Tab, closes on Escape or navigation, restores trigger focus, respects reduced motion, and collapses from the desktop two-register composition to a single mobile reading order. The live component is demoed on `/design-system`.
...
- `.editorial-breadcrumbs` — small-caps Archivo breadcrumb row with separators.
...
- `.editorial-page-meta` — small-caps Archivo meta strip.
...
- `.meth-figure` + `.meth-figure-caption` — wrapper for inline-SVG methodology charts (e.g. `<EigenvalueChart>` on the PCA appendix §4). Print-inspired card aesthetic: grid-cell background, card-border, small-caps Archivo caption.
