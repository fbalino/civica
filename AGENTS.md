...
- Nine-domain operational coverage is generated with `npm run generate:source-coverage`, checked into `src/lib/provenance/domain-coverage.generated.json`, and published at `/methodology/source-coverage` plus `/api/source-coverage`. Run `npm run validate:source-coverage` after elections, constitutions, offices, people, parties, organizations, bills, indicators, image, or source-freshness changes; use `npm run audit:source-coverage:live` to compare the checked report with Neon.
- The backup/PITR/release recovery procedure lives in `data/BACKUP-RESTORE-DRILL.md`; checked evidence is `plan/evidence/DAT-021/restore-drill.json`, enforced by `npm run validate:backup-restore`. Database dumps, WAL, base backups, restored clusters, secrets, and provider credentials must never enter the repository.
...
- **Typography**: Newsreader (serif, display + country cards) via `--font-heading`, Archivo (sans — body/interface/labels/eyebrows/numeric) via `--font-body`. Both are self-hosted under the SIL Open Font License. The production faces preserve the approved Type Lab calibration: Newsreader `size-adjust: 120%`, Archivo `size-adjust: 106%`, and the complete `--leading-*` scale is 120% of its prior values. Hero and country-masthead titles use `--font-weight-light`; H2 and lower editorial headings use `--font-weight-regular`. **Monospace (`--font-code`) is reserved for literal code/API snippets only** — never for labels, IDs, meta rows, eyebrows, tabs, or readable facts. (The legacy `--font-mono` token is repointed to Archivo so stragglers can't render monospace.) Small-caps labels = Archivo + uppercase + letter-spacing; aligned numeric columns = Archivo + `font-variant-numeric: tabular-nums`. Do not substitute other typefaces without an explicit owner decision.
...
- **Tabs**: use Archivo/body text with normal casing, matching Atlas (`Structure`, `Bills`, etc.). No Roman-numeral monospace tab labels.
