...
async function main() {
  ...
  const checked = JSON.parse(readFileSync("data/releases/ci-index-baselines-v1/manifest.v1.json", "utf8"));
  const errors: string[] = [];
  if (checked.schemaVersion !== "civica-index-baseline-manifest/v1") errors.push("wrong manifest schema");
