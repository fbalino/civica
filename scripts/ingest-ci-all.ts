...
async function main() {
  ...
  try {
    ...
    const combinedRows = stages.flatMap((stage) => stage.rows.map((row) => ({ jurisdictionId: row.jurisdictionId, dimension: row.dimension })));
    ...
    await sql.transaction((txn) => [
      txn`DELETE FROM ci_dimension_scores d WHERE d.quarter=${quarter} AND d.methodology_version=${methodologyVersion} AND d.dimension=ANY(${stagedDimensions}) AND NOT EXISTS (SELECT 1 FROM jsonb_to_recordset(${JSON.stringify(combinedRows)}::jsonb) AS x("jurisdictionId" uuid, dimension text) WHERE x."jurisdictionId"=d.jurisdiction_id AND x.dimension=d.dimension)`,
      ...stages.map((stage) => {
        const ingestionId = randomUUID();
        return txn`WITH ingestion AS (
          INSERT INTO ci_source_ingestions (id,source_id,dimension,dataset_year,native_scale_min,native_scale_max,is_inverted,global_min_observed,global_max_observed,countries_covered,status)
          VALUES (${ingestionId},${stage.sourceId},${stage.dimension},${stage.datasetYear},${stage.nativeScaleMin},${stage.nativeScaleMax},${stage.isInverted},${stage.globalMinObserved},${stage.globalMaxObserved},${stage.countriesCovered},'completed')
          ON CONFLICT (source_id,dimension,dataset_year) DO UPDATE SET native_scale_min=EXCLUDED.native_scale_min,native_scale_max=EXCLUDED.native_scale_max,is_inverted=EXCLUDED.is_inverted,global_min_observed=EXCLUDED.global_min_observed,global_max_observed=EXCLUDED.global_max_observed,countries_covered=EXCLUDED.countries_covered,ingested_at=NOW(),status='completed',error_message=NULL
          RETURNING id
        )
        INSERT INTO ci_dimension_scores (jurisdiction_id,dimension,quarter,normalized_score,raw_value,source_id,ingestion_id,methodology_version,derivation_version_key,derivation_versions)
        SELECT x."jurisdictionId",x.dimension,x.quarter,x."normalizedScore",x."rawValue",x."sourceId",ingestion.id,x."methodologyVersion",x."derivationVersionKey",x."derivationVersions"
        FROM jsonb_to_recordset(${JSON.stringify(stage.rows)}::jsonb) AS x("jurisdictionId" uuid,"normalizedScore" real,"rawValue" real,"sourceId" text,dimension text,quarter text,"methodologyVersion" text,"derivationVersionKey" text,"derivationVersions" jsonb), ingestion
        ON CONFLICT (jurisdiction_id,dimension,quarter,methodology_version) DO UPDATE SET normalized_score=EXCLUDED.normalized_score,raw_value=EXCLUDED.raw_value,source_id=EXCLUDED.source_id,ingestion_id=EXCLUDED.ingestion_id,derivation_version_key=EXCLUDED.derivation_version_key,derivation_versions=EXCLUDED.derivation_versions`;
      }),
