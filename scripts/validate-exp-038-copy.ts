...
for (const expectation of expectations) {
  ...
  for (const fragment of expectation.present) {
    if (!source.includes(fragment)) {
      errors.push(`${expectation.path}: missing approved/retained fragment: ${fragment}`);
    }
  }
  for (const fragment of expectation.absent ?? []) {
    if (source.includes(fragment)) {
      errors.push(`${expectation.path}: held/rejected fragment was applied: ${fragment}`);
    }
  }
}
...
console.log(
  "EXP-038 copy contract passed: approved edits are present and held copy remains unapplied.",
);
