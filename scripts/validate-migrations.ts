...
const errors = validateMigrationRegistry(MIGRATION_ARTIFACTS, sqlFiles, dataScripts, journal.entries.map((entry) => entry.tag), pkg.scripts);
...
console.log(`Forward artifacts: ${MIGRATION_ARTIFACTS.length} (${sqlFiles.length} SQL, ${dataScripts.length} operational data changes)`);
